/**
 * Alert — inline banner for errors, warnings, and success messages
 */
...
export default function Alert({ type = 'error', message, onDismiss, className = '' }) {
  ...
  return (
    <div className={`flex items-start gap-2.5 p-4 rounded-xl border text-sm ${v.wrapper} ${className}`}>
      {v.icon}
      <span className="flex-1 leading-relaxed">{message}</span>
      {onDismiss && (
        <button onClick={onDismiss} className="flex-shrink-0 opacity-60 hover:opacity-100 transition-opacity ml-1">
